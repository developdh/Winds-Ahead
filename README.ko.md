...
**도메인 준비:** 사용자가 가비아에 `windsahead.com`의 DNS를 입력했습니다. 기본 주소와 `www`의 도메인·HTTPS 연결이 활성화됐으며 브라우저에서도 비공개 사이트의 로그인 화면을 확인했습니다. [설정 안내](docs/ko/custom-domain.md)에 검증 상태를 기록했으며 아직 전체 공개하지 않았습니다.
