...
for(const c of data.cosmetics){for(let i=0;i<c.images.length;i++){
 ...
 await sharp(`${out}/${key}-full.webp`).resize(160,160,{fit:'inside',withoutEnlargement:true}).webp({quality:72}).toFile(`${out}/${key}-preview.webp`);
 records.push({cosmeticId:c.id,index:i,originalUrl:c.images[i].url,thumbnail:`/media/${key}-thumb.webp`,full:`/media/${key}-full.webp`,preview:`/media/${key}-preview.webp`,previewBytes:(await fs.stat(`${out}/${key}-preview.webp`)).size,thumbnailBytes:(await fs.stat(`${out}/${key}-thumb.webp`)).size,fullBytes:(await fs.stat(`${out}/${key}-full.webp`)).size,permission:'unknown',audience:'owner-private reference preview'});
}}
