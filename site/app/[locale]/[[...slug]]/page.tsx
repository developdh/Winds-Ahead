...
export async function generateMetadata({ params }: Props) {
  ...
  const item = route.itemId && findCosmetic(route.itemId);
  const title = item
    ? item.nameOriginal
  ...
  return {
    ...
    description:
      locale === "ko"
        ? "공식 중국 자료로 살펴보는 연운 외관 도감. 글로벌 정보와 예상 일정을 구분해 확인하세요."
        : "Explore Where Winds Meet cosmetics with official CN sources, global status, and clearly separated forecasts.",
    alternates: {
      languages: {
        en: `/en/${(slug ?? []).join("/")}`,
        ko: `/ko/${(slug ?? []).join("/")}`,
